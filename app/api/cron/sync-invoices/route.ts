...
/**
 * API route handler for invoice sync cron job.
 * Verifies CRON_SECRET if configured, then syncs invoices from Rubic to Tripletex.
 */
export async function GET(request: Request) {
	...
	try {
		...
		// Run sync
		const result = await syncInvoices(rubicClient, tripletexClient, db);
